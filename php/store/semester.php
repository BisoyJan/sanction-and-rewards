...
if (isset($_GET['select_semesters'])) {
    if (empty($_SESSION['school_year'])) {

        $select_semester = mysqli_real_escape_string($con, $_GET['select_semesters']);

        $query = "SELECT * FROM `semesters` WHERE school_year LIKE '%$select_semester%'";
        $query_run = mysqli_query($con, $query);
