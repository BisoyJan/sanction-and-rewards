...
if (isset($_GET['sanction_NotificationWarning'])) {
    $query = "SELECT
        COUNT(offenses.id) AS total,
        offenses.offense
    FROM
    ...
    JOIN offenses ON violations.offenses_id = offenses.id
    JOIN programs ON students.program_id = programs.id
    WHERE sanction_referrals.remark IS NULL AND (offenses.offense = 'Serious Offense' OR offenses.offense = 'Very Serious Offense')
    GROUP BY
        offenses.offense";
    $query_run = mysqli_query($con, $query);
